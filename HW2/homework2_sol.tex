...
$$= \prod_{j=1}^K (p(\boldx_i | \boldy_i = C_j)\pi_j)^{\boldy_{ij}}$$

where $\boldy_{ij}$ is the $j$th element of $\boldy_i$. This makes the likelihood of the data set
$$p(D; \bpi) = p( \{(\boldx_i, \boldy_i)\}_{i=1}^N; \pi)$$
$$ = \prod_{i=1}^N\prod_{j=1}^K (p(\boldx_i | \boldy_i = C_j)\pi_j)^{\boldy_{ij}}$$

and the log-likelihood of the data set

$$\ln p(D; \bpi) = \sum_{i=1}^N \sum_{j=1}^K \boldy_{ij}(\ln (p(\boldx_i | \boldy_i = C_j)\pi_j)$$
$$ = \sum_{i=1}^N \sum_{j=1}^K \boldy_{ij}(\ln p(\boldx_i | \boldy_i = C_j) + \ln\pi_j)$$
...
Taking the derivative with respect to $\bpi$ and setting it to zero yields

$$\frac{\partial L(\bpi, \lambda)}{\partial  \bpi} 
=  \frac{1}{\pi_j} \sum_{i=1}^N y_{ij} + \lambda = 0$$

$$\implies \pi_k = -\frac{1}{\lambda} \sum_{n=1}^N y_{nk} = -\frac{N_k}{\lambda}$$

\item Isolating $\bmu_k$ in the log-likelihood yields the following
