...
\begin{enumerate}
  ...
  \item Need to change so that $x_i$ is not included inside of nested summation
  ...
  \begin{equation*}
    = \sum_{i} \left(y_i - \sum_{n} \exp\left(\frac{-(x_n-x_i)^T (x_n-x_i)}{\tau} \right) y_n\right)^2 
  \end{equation*}

  In the above I intentionally include the term 
  $K(x_n,x_i)y_n$ in the definition of $f(x_i)$. The non-parametric
  model should not change when given different inputs.

  \item 
  
  \begin{equation*}
    \frac{d}{d\tau} \mcL(W) = \frac{d}{d\tau} \sum_{i} \left(y_i - \sum_{n} \exp\left(\frac{-(x_n-x_i)^T (x_n-x_i)}{\tau} \right) y_n\right)^2 
  \end{equation*}

  \begin{equation*}
    = 2 \sum_{i} \left(y_i - \sum_{n} \exp\left(\frac{-(x_n-x_i)^T (x_n-x_i)}{\tau} \right) y_n\right) 
    \left( - y_n \sum_{n} \exp\left(\frac{-(x_n-x_i)^T (x_n-x_i)}{\tau} \right) \right) 
  \end{equation*}
